...
[FOreg,FOregi,Alims,HEAD,DN_out]=wera_imageFOLs_v5(data,CONST.whichant_FOL,SpecHead.iFBraggc,CONST.v_incr,CONST.imageFOL_user_param,CONST.goplot,HEAD);
...
%         [R, HEAD]=HFR_DP_lera_music_v3(data,FOregi,HEAD,SpecHead,patt,CONST);
        [R, HEAD]=HFR_DP_wera_music_v3(data,FOregi,HEAD,SpecHead,patt,CONST);
    else
...
if length(R)>100;
    ...
    disp(['not enough returns program ending'])
    disp([num2str(length(R)) ' solutions'])
end %length(R) >500
