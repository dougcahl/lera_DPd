...
%%% load the file
load(filein);
% RC=RCc;  %reload the radar_header version of the RC (only part for WERA files)
% RC.R = RCc.R;
% RC.Range_Res_km = RCc.Range_Res_km;
RCc.NCHIRP = RC.NCHIRP;
RC=RCc;
...
%%%%%%%%%%%%%%%
%flip IQ channels... not for WERA
% timechirp=timechirp([2 1],:,:,:); % moved to RAW section
swpdir='up'; % only for raw files
...
SpecHead.nchirps=RC.NCHIRP;           %# of chirps captured in the TS file, could be variable

% Rkm = R
% Rm = r
SpecHead.RangeMaxKm = RC.R(end);
SpecHead.RangeResKm = RC.Range_Res_km;
SpecHead.FullRangeKm = RC.R/10^3;
...
%%
if ~exist('I','var') % assume decimated WERA .RAW file
    ...
    for ichirp=1:file_chirps
        ...
        %if upsweep, reverse range coordinate to account for sweep direction
        %difference
        if strcmp(swpdir,'up')==1 % accounted for with "irange"
            Y=flipud(Y);
        end
...
else % assume from WERA .SORT file
    %% This is where the WERA file comes in %%
    SpecHead.nchirps = WERA.SAMPZ;
    SpecHead.nranges = WERA.NRRANGES;
    ...
    if SpecHead.nsegs > 1
        ...
        for ii=1:RC.NANT
            for iii=1:SpecHead.nsegs;
                b=SpecHead.segments(iii):SpecHead.segments(iii)+SpecHead.PC.SpeclengthPnts-1;   %adjusted v4 for flexibility
                y = squeeze(I(ii,irange,b)+1i*Q(ii,irange,b));
                Y=fft(y.*(ones(length(irange),1)*(hanning(SpecHead.PC.SpeclengthPnts)')),[],2);   %do the fft with a hamming window
                Y=fftshift(Y,2);  %shift to put the 0 frequency in the middle.
                %try normalizing each antenna by the rms power in a range around the center point
                nabs=sqrt(nanmean(abs(Y(normpt(1),[normpt(2)-floor(SpecHead.PC.SpeclengthPnts/500):normpt(2)+floor(SpecHead.PC.SpeclengthPnts/500)])).^2));
