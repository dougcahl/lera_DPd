...
CONST.site_name='gtn';
CONST.Site_loc = [33.3561 -79.1528]; % [lat lon]
CONST.Site_bounds=[98-70 98+70]; % in degT
...
CONST.whichant_FOL=9; % can be a single antenna or a group of antennas
...
CONST.gohitch=1;    %find and remove a consistent 'hitch' in the chirps (only for RAW files)
...
CONST.PC=[];
CONST.PC.MaxRangeKM=240;        % the maximum ranges, in distance to be kept in spectra, from >0 to Max
...
%%%%%%%%%
% change this
CONST.PC.SpeclengthPnts=1024; %1024;   %defines the number of points in the spectra
...
start_time=datenum(2019,11,1,0,0,0);  % example data -- 3 files from .RAW and one from .SORT
end_time=datenum(2019,11,20,17,0,0); 
...
HFR_DP_wera_ts2radial_prepwork_v6 % for WERA data
...
%     [SpecHead,data,mtime]=lera_time2spectra_v7(incoming_ts_file_dir,incoming_spectra_file_dir,filein,RC,HEAD,CONST);
    [SpecHead,data,mtime]=wera_time2spectra_v7(incoming_ts_file_dir,incoming_spectra_file_dir,filein,RC,HEAD,CONST);
    ...
    elseif strcmp(RC.radar_type,'WERA')==1;
        fileout=['WERA_' RC.SiteName '_' filein(1:i(1)-1) '.mat'];
    end
    %%%%% save this data
    ...
    if isempty(data)==1;  %something is wrong with this file...
        ...
        if CONST.goplot(1)==1;
            ...
            for ii=1:RC.NANT
                ...
                subplot(4,3,ii);
                pcolor(v,r,20*log10(d)); shading flat; hg; caxis([-170 -80]);
...
%         HFR_DP_lera_spectra2radialmetric_process_v6    %update to add constants for fols
        HFR_DP_wera_spectra2radialmetric_process_v6    %update to add constants for fols for WERA data
...
%             figure(3); clf;
            ...
            m_quiver(RM.data(:,1),RM.data(:,2),RM.data(:,3),RM.data(:,4),1,'r');
...
%             [RADIAL,HEAD]=HFR_DP_lera_RadialAverage_v2(RM,patt,HEAD,CONST);   %update to do different weighting on radave velocities
            [RADIAL,HEAD]=HFR_DP_wera_RadialAverage_v2(RM,patt,HEAD,CONST);   %update to do different weighting on radave velocities for WERA data
...
%                 figure(4); clf;
                ...
                m_quiver(RADIAL.LonLat(i,1),RADIAL.LonLat(i,2),RADIAL.U(i,2)*sc,RADIAL.V(i,2)*sc,0,'r')
